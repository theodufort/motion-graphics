...
if (asJson) {
  console.log(JSON.stringify({ ts: new Date().toISOString(), score, max: existingMax + smokeMax, graphics: graphics.length, graphicsScores, graphicsTimings, graphicsLabels, graphicsWarnings, warnings: Object.entries(graphicsWarnings).flatMap(([n, ws]) => ws.map((w) => n + ": " + w)), problems }));
} else {
