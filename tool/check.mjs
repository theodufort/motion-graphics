...
async function worker() {
  while (cursor < graphics.length) {
    ...
    score += pts;
    if (pts < 35 || r.resize === 0 || r.content === 0) problems.push(`${name}: ${pts}/35 (${(r.errors || []).join("; ") || (r.resize === 0 ? "resize check failed" : r.content === 0 ? "content check failed" : "check bits failed")})`);
  }
}
